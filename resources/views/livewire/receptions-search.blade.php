    ...
    <div class="container-fluid">
        ...
        <li>
            <?php if(isset($_GET['name'])){?>
                    ...
                    <input type="text" wire:model="name" class="form-control"  placeholder="Recherche" value="{{ $name}}" readonly>
...
<div class="container-fluid" >
        ...
        <tr class="text-center"class="btn btn-raised btn-info btn-sm" @if ($reception->state=="Annulé")
            style="color:red"
        @endif
        ...
        >
                        ...
                        <td onclick="document.location='receptions/{{$reception->id}}'" >{{ $reception->name }}</td>
                        <td onclick="document.location='receptions/{{$reception->id}}'" >
                        ...
                        <td onclick="document.location='receptions/{{$reception->id}}'" >{{ $reception->date_shippement }}</td>
                        <td onclick="document.location='receptions/{{$reception->id}}'" >{{ $reception->purchase_order->name }}</td>
                        <td onclick="document.location='receptions/{{$reception->id}}'" >
                        ...
                        <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#exampleModal{{$reception['id']}}">
                                    ...
                                    <div class="modal-dialog modal-lg" role="document">
                                        ...
                                        <div class="modal-body">
                                            @if ($reception->state=="Reçu" OR $reception->state=="assigned")
                                                vous ne pouvez pas supprimer une réception @if ($reception->state=="Reçu")
                                                ayant déja été  réçu!
                                                @else
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            @else
                                            <form action="/receptions/{{ $reception['id'] ?? ''}}" method='post'>
                                                @method('delete')
                                                @csrf
...
@foreach($receptions as $reception)
<script>
$("#exampleModal{{$reception['id']}}").on('shown.bs.modal', function () {
  $("#myInput{{$reception['id']}}").trigger('focus')
})
</script>
@endforeach
@endsection
